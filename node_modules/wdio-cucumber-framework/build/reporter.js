...
var CucumberReporter = function () {
    ...
    (0, _createClass3.default)(CucumberReporter, [{
        ...
        value: function handleBeforeFeature(event, callback) {
            var feature = event;
        ...
        value: function handleBeforeScenario(event, callback) {
            var scenario = event;
        ...
        value: function handleBeforeStep(event, callback) {
            var step = event;
            this.testStart = new Date();
        ...
        value: function handleStepResult(event, callback) {
            ...
            var error = {};
            var stepTitle = step.name || step.keyword || 'Undefined Step';

            /**
             * if step name is undefined we are dealing with a hook
             * don't report hooks if no error happened
             */
            if (!step.name && stepResult.status !== _get__('Status').FAILED) {
                return process.nextTick(callback);
            }

            if (stepResult.status === _get__('Status').UNDEFINED) {
            ...
            } else if (stepResult.status === _get__('Status').FAILED) {
                /**
                 * cucumber failure exception can't get send to parent process
                 * for some reasons
                 */
                var err = stepResult.failureException;
                ...
                this.failedCount++;
            } else if (stepResult.status === _get__('Status').AMBIGUOUS && this.options.failAmbiguousDefinitions) {
                e = 'fail';
                this.failedCount++;
                error = {
                    message: 'Step "' + stepTitle + '" is ambiguous. The following steps matched the step definition',
                    stack: stepResult.ambiguousStepDefinitions.map(function (step) {
                        return step.pattern.toString() + ' in ' + step.uri + ':' + step.line;
                    }).join('\n')
                };
            }
        ...
        value: function handleAfterScenario(event, callback) {
            var scenario = event;
            this.emit('suite:end', {
                uid: this.getUniqueIdentifier(scenario),
                title: scenario.name,
        ...
        value: function handleAfterFeature(event, callback) {
            var feature = event;
            this.emit('suite:end', {
                uid: this.getUniqueIdentifier(feature),
                title: feature.name,
        ...
        value: function getUniqueIdentifier(target) {
            return target.name + (target.line || '');
        }
    }]);
